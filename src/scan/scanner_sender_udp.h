...
#include "scan/udp_ping_packet.h"

namespace turbo_links::scan
{


class scanner_sender_udp_params
{
public:
    scanner_sender_udp_params(
    ...
    const ping_packet_data_t ping_packet_data;
};

struct scanner_sender_udp_state
{
public:
    boost::asio::ip::udp::endpoint target_endpoint;
    std::uint32_t current_port_{0};
    std::uint32_t current_loop_{0};
    std::array<std::uint8_t, ping_packet_t::wire_size> send_buffer;
};

// scanner_udp 的独立发送任务，只发送固定长度的 PING。
class scanner_sender_udp
{
public:
    // socket 必须已绑定且未 connect；扫描总超时由上层 scanner_udp 管理。
    scanner_sender_udp(boost::asio::ip::udp::socket& _socket);
    ~scanner_sender_udp() = default;

    // 接收 scanner 的发送请求并开始扫描。
    // 一次请求严格发送 target_port, 1, 2, ... 65535，然后退出。
    // 即配置端口在遍历中仍会再发送一次，不探测 0 端口。
    void do_send(std::shared_ptr<scanner_sender_udp_params> _params);
...
protected:
    ...
    std::shared_ptr<scanner_sender_udp_params> params_;
    std::shared_ptr<scanner_sender_udp_state> state_;
